...
edge *is* a ``TransformationViz``, with no separate edge type. ``stateA``,
``stateB`` and ``protocol`` are gufe keys either way - resolved in the
transformation's own registry when it stands alone, in the network's when it is
an edge.

What repeats in a network is not the nodes and edges but what they are made of:
every system shares one protein and every transformation one protocol, so those
live in the registry once, as whole objects.
"""
...
def transformation_payload(transformation: TransformationBase, registry: Registry | None = None) -> dict[str, Any]:
    ...
    ``stateA`` and ``stateB``, both its single system, so it comes out as a diff
    with no differences. The two are siblings under ``TransformationBase``, which is
    why this dispatches on the base class.
    """
...
def alchemical_network_payload(network: gufe.AlchemicalNetwork) -> dict[str, Any]:
    ...
    ``AlchemicalNetwork`` has no ``to_graphml()``, so the graph is walked here. Nodes
    are registered before edges, which makes "every edge endpoint names a node" true
    by construction. Both are sorted by gufe key for byte-stability.
    """
