/**
 * Put a `<alchemy-view>` in a host element and give it a payload.
 *
...
/** The shape a `<alchemy-view>` presents to whoever mounted it. */
export type MountedView = HTMLElement & { payload: unknown };

/**
 * Reuses an existing `<alchemy-view>` if `host` already has one, so calling this
...
export function mount(host: HTMLElement, payload?: unknown): MountedView {
  let view = host.querySelector("alchemy-view") as MountedView | null;
  if (!view) {
    view = document.createElement("alchemy-view") as MountedView;
